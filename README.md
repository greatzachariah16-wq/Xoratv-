...
- **User Video Hosting & 240p Streaming**: User video uploads are uploaded directly to **Cloudinary** using an unsigned upload preset and delivered with a forced **~240p transformation** (`h_240,c_scale,q_auto:low,f_mp4`). Long catalog movies remain provider embeds (YouTube/Vimeo/Dailymotion). Render disk upload remains available only as an unconfigured fallback.
...
| `VITE_CLOUDINARY_UPLOAD_PRESET`     | Cloudinary unsigned upload preset name                         | `xora_videos`                                 |
| `VITE_CLOUDINARY_FOLDER`            | Cloudinary target folder                                       | `xora/videos`                                 |
...
## Cloudinary Video Hosting & ~240p Delivery Setup

User-created videos are hosted on **Cloudinary**, replacing Render's ephemeral disk storage. All post metadata remains in **Firebase Realtime Database (RTDB)**.

### Implemented Strategy: Option 1 (Cloudinary On Upload with 240p Transformation)

- **Duration Cap**: Maximum 30 minutes (1800 seconds). Validated client-side via HTML5 video metadata before upload and rejected with a toast if exceeded.
- **Size Cap**: Maximum 100 MB. Validated client-side and rejected with a toast if exceeded.
- **240p Delivery**: Cloudinary transformation `h_240,c_scale,q_auto:low,f_mp4` is embedded in the post's `stream_url`. Videos are scaled down to 240p and compressed for minimal bandwidth consumption.
- **Catalog Movies**: Provider embeds (YouTube, Vimeo, Dailymotion) continue to stream via their official embedded players.

### Step-by-Step Cloudinary Configuration:

1. **Sign Up / Log In**:
   - Go to [Cloudinary](https://cloudinary.com/) and create or log in to your account.
2. **Retrieve Your Cloud Name**:
   - In the Cloudinary Console dashboard, copy your **Cloud name** (e.g. `dxy...`).
3. **Create an Unsigned Upload Preset**:
   - Go to **Settings** (gear icon) > **Upload** tab.
   - Scroll down to **Upload presets** and click **Add upload preset**.
   - Preset name: enter `xora_videos` (or your preferred name).
   - **Signing Mode**: Change from _Signed_ to **Unsigned**.
   - **Folder**: Enter `xora/videos`.
   - (Optional) Under **Media analysis and AI**, or Transformations, you can keep defaults or set incoming constraints.
   - Click **Save**.
4. **Configure Environment Variables in Render**:
   - In the Render Dashboard under your Web Service's **Environment** tab, set:
     - `VITE_CLOUDINARY_CLOUD_NAME` = `<your_cloud_name>`
     - `VITE_CLOUDINARY_UPLOAD_PRESET` = `xora_videos`
     - `VITE_CLOUDINARY_FOLDER` = `xora/videos`
5. **Bandwidth Notice**:
   - Even delivered at 240p, 30-minute videos consume bandwidth. Cloudinary's free tier provides 25 monthly credits (~25 GB bandwidth / transformations). Monitor usage on the Cloudinary dashboard if upload volume is high.
...
6. _Note on Verification_: For internal or test users, branding displays immediately. For general public users outside your workspace/organization, Google may review the app name and logo before public verification is granted.
...
```json
{
  "rules": {
    ...
    "supportTickets": {
      ...
      ".indexOn": ["user_id", "status", "created_at", "priority"],
      "$ticketId": {
        ".read": "auth != null && (root.child('admins').child(auth.uid).val() === true || data.child('user_id').val() === auth.uid)",
        ".write": "auth != null && ( (!data.exists() && newData.child('user_id').val() === auth.uid) || (data.exists() && root.child('admins').child(auth.uid).val() === true) )",
        ".validate": "newData.hasChildren(['id','user_id','category','subject','message','status','priority','created_at','updated_at']) && newData.child('id').isString() && newData.child('user_id').isString() && newData.child('category').isString() && (newData.child('category').val() === 'technical' || newData.child('category').val() === 'account' || newData.child('category').val() === 'billing' || newData.child('category').val() === 'report' || newData.child('category').val() === 'privacy' || newData.child('category').val() === 'feedback' || newData.child('category').val() === 'other') && newData.child('subject').isString() && newData.child('subject').val().length > 0 && newData.child('message').isString() && newData.child('message').val().length > 0 && newData.child('status').isString() && (newData.child('status').val() === 'open' || newData.child('status').val() === 'pending' || newData.child('status').val() === 'resolved' || newData.child('status').val() === 'closed') && (!data.exists() ? newData.child('status').val() === 'open' : true) && newData.child('priority').isString() && (newData.child('priority').val() === 'normal' || newData.child('priority').val() === 'high') && newData.child('created_at').isNumber() && newData.child('updated_at').isNumber() && (!data.exists() || (newData.child('id').val() === data.child('id').val() && newData.child('user_id').val() === data.child('user_id').val() && newData.child('created_at').val() === data.child('created_at').val()))"
      }
    }
  }
}
...
> **Note on Chat Rules**: In Firebase Console > Realtime Database > Rules, apply the rules above. For production strictness, ensure `chatMessages` and `chatRooms` only permit reads and writes by authenticated members of the respective room (`data.child('memberIds/' + auth.uid).exists()`).

---

## Live Chat & 7-Day Auto-Deletion

Xora features a real-time Chat system for direct messages, creator discussions, and watch experiences:

1. **Routes**:
   - `/chat`: Inbox, conversation switcher, New DM modal, New Group modal, Watch Experience share.
   - `/chat/$roomId`: Direct link to a specific room or conversation.

2. **Strict 7-Day Message Expiry**:
   - Every message stores `expiresAt` set to `created_at + 7 days`.
   - **Client-side prune on room read**: When viewing a room, messages older than 7 days are filtered from view and automatically purged from RTDB.
   - **Client-side prune on inbox open**: When opening `/chat`, user rooms are opportunistically cleaned.
   - **Server-side purge endpoint**: Optional endpoint `GET /api/chat/purge` removes all expired messages in RTDB.

---

## Legal & Google OAuth Compliance Pages

- **Privacy Policy**: `https://xoratv-x.onrender.com/privacy`
- **Terms of Service**: `https://xoratv-x.onrender.com/privacy#terms`
- Both sections are publicly crawlable and fulfill Google Cloud OAuth Consent Screen verification requirements.
