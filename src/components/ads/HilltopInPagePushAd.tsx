...
export function HilltopInPagePushAd() {
  useEffect(() => {
    ...
    try {
      (function (pnooxz: Record<string, unknown>) {
        const d = document;
        const s = d.createElement("script");
        const l = d.currentScript || d.scripts[d.scripts.length - 1];
        s.id = SCRIPT_ID;
        (s as unknown as { settings: Record<string, unknown> }).settings = pnooxz || {};
        s.src =
